<script lang="ts">
	...
	let checkSaltInterval;
	let previousSalt = $salt;
	...
	onMount(async () => {
		...
		checkSaltInterval = setInterval(async () => {
			await currentDifficultyAndSalt();
			if (previousSalt !== $salt) {
				console.log('Salt has changed. Restarting mining process...');
				terminateWorkers();
				//Clean result
				results = [];
				startMining();
	...
	onDestroy(() => {
		clearInterval(checkSaltInterval);
	});
	...
	<article class="bg-gray-900">
		<div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16">
			...
			<p class="mb-8 text-lg font-normal text-gray-500 dark:text-gray-400">
				You must find a nonce that when doing <code class="display-inline!important"
					>keccak256(encodePacked(YOUR_WALLET,SALT,NONCE))</code
				>
			...
			{#each results as data}
				<div class="font-mono">
					...
					<img src="/images/{data.buttplug}.gif" class="w-52 h-52" alt="Buttplug {data.buttplug}" />
				</div>
				<button
					class="btn"
					on:click={() => {
