      ...
      for (let i = 0; i < workerCount; i++){
        ...
        w.onmessage = (e) => {
            ...
            if (response[1] != -1) {
                ...
                const message = BigInt(keccak256(
        ...
        {:else}
            ...
            {#each results as data}
                <div class="font-mono">
                    ...
                    <img src="https://bafybeibnw2yuc7tpkt4pkzx3c2yizyjx24vioehwqodxppbqoyncyi4t44.ipfs.dweb.link/{data.buttplug}.gif" alt="Buttplug {data.buttplug}" />
                </div>
                <button class="btn" on:click={() => { mintButtplug(data.nonce) }}>Mint Buttplug {data.buttplug}</button>
                <hr />
            {/each}
