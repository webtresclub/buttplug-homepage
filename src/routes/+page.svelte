<script lang="ts">
	...
	const fetchTopTen = async () => {
		const query = `query {
            buttpluggyNfts(first: 10, orderBy: id, orderDirection: desc){
                id
            }
        }`;
		...
		try {
			...
			topTenIds = responseData.data.buttpluggyNfts.map((nft) => nft.id);
		} catch (e) {
...
<main class="container">
	{#if topTenIds.length}
		<h3>These are the last minted Buttpluggies</h3>

		<div class="space-y-8 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-12 md:space-y-0">
			{#each topTenIds as id}
