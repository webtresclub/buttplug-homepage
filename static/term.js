function boot(fullId){
    ...
    function enter(){
        ...
        hint.style.opacity=0;
        setTimeout(()=>{splash.remove(); bootTerminal();},600);
    }
    ...
    printLink("[Buttpluggy #"+fullId+"] on OpenSea", "https://opensea.io/item/ethereum/0x0000420538cd5abfbc7db219b6a1d125f5892ab0/"+ String(parseInt(fullId)));
        /* utilities */
        ...
        function clock(){
            ...
            function tick(){
                ...
                ctx.fillStyle="#0f0";ctx.font="64px monospace";ctx.textAlign="center";
                ctx.fillText(new Date().toLocaleTimeString(),c.width/2,c.height/2);
                clockRAF=requestAnimationFrame(tick);
            }tick();
            const esc=e=>{if(e.key==="Escape"){cleanup();}};
            function cleanup(){cancelAnimationFrame(clockRAF);c.remove();
                window.removeEventListener("resize",resize);document.removeEventListener("keydown",esc);}
            document.addEventListener("keydown",esc);
        }
        /* screensaver */
        function screensaver(){
            const logo=new Image();logo.src=gif.src;
            const c=document.createElement("canvas");const ctx=c.getContext("2d");
            let x=100,y=100,vx=3,vy=3,size=128;
            function resize(){c.width=innerWidth;c.height=innerHeight;}
            resize();window.addEventListener("resize",resize);
            document.body.appendChild(c);
            function step(){
        ...
        inp.addEventListener("keydown",e=>{
            ...
            switch(cmd){
                ...
                case "ping": printLine("pong"); break;
                case "gif": printLine("(re-displaying)"); out.appendChild(gif.cloneNode()); break;
                case "clock": clock(); break;
                case "screensaver": screensaver(); break;
                case "exit": location.reload(); break;
                default: printLine("unknown command"); break;
            }
            // clear input 
            inp.value = "";
        });
